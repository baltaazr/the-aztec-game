...
namespace the_aztec_game
{
    class Game
    {
        ...
        public void start()
        {
            ...
            typewriterStyleOutput("What is your player's occupation?: ", "self");
...
");
            ...
            if (move)
            {
                ...
                if (enemyAppeared)
                {
                    double playerLuck = player.getStatsWArmor()["luck"];
                    ...
                    else
                    {
                        ...
                        else ENEMY_D = Configs.ENEMY_DIFFICULTY[5];
                    }
...
{0}:
  {1}
{2}
", enemy.name, enemy.description, enemy.image), "enemy");
            if (player.speedRoll())
            {
                typewriterStyleOutput(@"
...
", i, possibleMove.Key));
            ...
            if (possibleMoves[intIndexToStringIndex[moveIndex - 1]] == -1)
            {
...
");
                ...
                if (intIndexToStringIndex[moveIndex - 1] == "Punch")
                {
                    dmgDealt = player.getRandPunchDamage() * player.stats["dmgmod"];
                }
            }
            else
            {
                Item itemUsed = player.inventory[possibleMoves[intIndexToStringIndex[moveIndex - 1]]];
                if (itemUsed is Weapon)
                {
                    dmgDealt = itemUsed.getRandDmg() * player.stats["dmgmod"];
                }
                else
                {
                    typewriterStyleOutput(string.Format(@"

You've consumed your {0}, these are your stats now
", itemUsed.name));
                    player.consumeConsumable(possibleMoves[intIndexToStringIndex[moveIndex - 1]]);
                    displayStats();
                    return false;
                }

            }
            if (player.courageRoll())
            {
                typewriterStyleOutput(@"

Because of you're great courage, you deal an abnormal amount of damage to the enemy.
");
                dmgDealt *= 5;
            }
            typewriterStyleOutput(string.Format(@"
...
Shortly after, you woke up realizing you had lost {0}.
...
", player.getStringStats()), "stats");
        ...
        private void typewriterStyleOutput(string message, string type = "default")
        {
            for (int i = 0; i < message.Length; i++)
            {
                switch (type)
                {
                    ...
                    case "enemy":
                        ...
                        break;
                    case "warn":
                        Console.Write(message[i], Color.FromArgb(94, 223, 255));
                        break;
                    case "stats":
                        Console.Write(message[i], Color.Orange);
                        break;
                    case "self":
                        Console.Write(message[i], Color.PeachPuff);
                        break;
                    case "others":
                        Console.Write(message[i], Color.LightGreen);
                        break;
                    default:
                        Console.Write(message[i], Color.FromArgb(255, 255, 255));
                        break;
                }
...
You increased your Damage by {0}
         ", (player.stats["hp"]), (player.stats["dmgmod"])));
         player.stats["hp"] *= 2;
         player.stats["dmgmod"] *= 2;
